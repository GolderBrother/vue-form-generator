...
const tags = {
  ...
  'el-tag': el => {
    ...
    const effect = el.effect ? `effect="${el.effect}"` : ''
    const size = el.size ? `size="${el.size}"` : ''
    const closable = el.closable ? `:closable='${el.closable}'` : ''
    const hit = el.hit ? `:hit='${el.hit}'` : ''
    const disableTransitions = el.disableTransitions ? `:disable-transitions='${el.disableTransitions}'` : ''
    let child = buildElTagChild(el, {
      type,
      color,
      effect,
      size,
    ...
    if (child) child = `\n${child}\n` // 换行
    // return `<${tag} ${type} ${color} ${size} ${effect} ${hit} ${closable} ${disableTransitions}>${child}</${tag}>`
    return `${child}`
  },
  'el-input': el => {
    const {
...
// el-tag 子级
function buildElTagChild(scheme, attrs) {
  const children = []
  const slot = scheme.__slot__ || {}
  const {
    type,
  ...
  if (Array.isArray(slot.tags) && slot.tags.length) {
    children.push(`<el-tag v-for="(item, index) in ${scheme.__vModel__}Tags" :key="index" ${type} ${color} ${size} ${effect} ${hit} ${closable} ${disableTransitions}>{{item.label}}</el-tag>`)
